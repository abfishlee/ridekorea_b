...
> 최종 업데이트: 2026-07-01 · 상태: 로컬 개발 환경 구동 확인, 전체 진행률 **≈76%**
...
- **지금 상태**: 백엔드(RPC·RLS) 대부분 완성 + 로컬 DB 검증 완료. 프론트 Explore/Detail 완성, **Phase 4 Ride 화면(글래스 대시보드 + 라이브 지도 + 카메라 핀) 방금 완성**.
- **다음 할 일**: ① 웹 Ride 화면 실지도화 + "특정 루트 라이딩(이탈→finalize)" 연결. (§7 참고)
...
**전체 ≈76% (25/33 작업).** 정확한 최신 상태는 항상 `vision_doc/10_development_phases.md` 참조.
...
**방금 완료 (Phase 4 / Dev Client 준비)**
- `app/(tabs)/ride.tsx` — 자유 라이딩 시작/일시정지/종료 + 복구 프롬프트
...
**➡️ 다음 (우선순위)**
1. **① 웹 Ride 화면 실지도화** — `RideMap.web.tsx`에 인라인 웹 네이버 지도, 브라우저 geolocation으로 추적 테스트.
2. **"특정 루트 라이딩" 연결** — 루트 상세(`app/route/[id].tsx`)에서 planned line + routeId를 Ride 화면으로 넘겨 **이탈(파랑→분홍) + `finalize_ride`** 실제 플로우 완성. (지금 Ride 탭은 자유 라이딩 = 이탈/finalize 미노출)
3. 잔여 백엔드 곁가지: 1.4 Edge Function 통합테스트, 7.2 logistics_tips.
